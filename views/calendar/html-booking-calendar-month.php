 ...
 */
if ( $_product && ! $_product->is_type( YITH_WCBK_Post_Types::PRODUCT_TYPE ) ) {
	$_product = false;
}
		...
		<?php while ( $timestamp <= $end_timestamp ) : ?>
			...
			<div class="yith-wcbk-booking-calendar-day-container">
				...
				<div class="bookings">
					<?php
					/**
					 * DO_ACTION: yith_wcbk_calendar_month_bookings_list_before
					 * Hook to output something before bookings' list in the date of the month calendar.
					 *
					 * @param int $timestamp The timestamp of the current date.
					 */
					do_action( 'yith_wcbk_calendar_month_bookings_list_before', $timestamp );
					?>
					<?php
