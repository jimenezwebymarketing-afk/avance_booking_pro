        ...
        <div class="booking-data__column">
            ...
            <div class="form-field form-field-wide yith-booking-order">
                <label for="yith_booking_order"><?php esc_html_e( 'Order:', 'yith-booking-for-woocommerce' ); ?>
                    ...
                    if ( $order_id ) {
                        $order_related = wc_get_order( $order_id );
                        if ( $order_related ) {
                            $order_link = is_object( $order_related ) && method_exists( $order_related, 'get_edit_order_url' ) ? $order_related->get_edit_order_url() : get_edit_post_link( $order_id );
            ...
            <?php $user_id = $booking->get_user_id( 'edit' ); ?>
            <div class="form-field form-field-wide yith-booking-user">
                <label for="yith_booking_user"><?php esc_html_e( 'User:', 'yith-booking-for-woocommerce' ); ?>
                    <?php do_action( 'yith_wcbk_booking_metabox_info_user', $booking, $user_id ); ?>
                    <?php if ( current_user_can( 'yith_manage_bookings' ) ) : ?>
                        <?php
                        if ( $user_id ) {
                            $edit_link = get_edit_user_link( $user_id );
                            echo wp_kses_post( sprintf( '<a href="%s">%s &rarr;</a>', $edit_link, __( 'View user', 'yith-booking-for-woocommerce' ) ) );
                        }
                        ?>
                    <?php endif; ?>
                ...
                if ( current_user_can( 'yith_manage_bookings' ) ) {
                    yit_add_select2_fields(
                        array(
                            ...
                            'data-placeholder' => apply_filters( 'yith_wcbk_booking_user_placeholder', __( 'N.D.', 'yith-booking-for-woocommerce' ) ),
                            'data-allow_clear' => true,
                            'data-multiple'    => false,
                            'value'            => $user_id,
                            'data-selected'    => $data_selected,
            ...
            /**
             * APPLY_FILTERS: yith_wcbk_admin_booking_show_calculated_amount
             *
             * Filters if showing the calculated amount for the booking (admin side).
             *
             * @param bool              $show    True to show; false otherwise.
